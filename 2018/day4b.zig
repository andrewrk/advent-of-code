...
pub fn main() !void {
    ...
    std.debug.warn("{}\n", try guard(allocator, main_input));
}
...
test "sample inputs" {
    ...
    ;
    const result = try guard(std.debug.global_allocator, sample_input);
    std.debug.assert(result == 4455);
}

fn lexicalLessThan(lhs: []const u8, rhs: []const u8) bool {
...
fn parseLine(line: []const u8) !Msg {
    var space_it = std.mem.split(line, " ");
    _ = space_it.next().?; // yyyy-mm-dd
    const time = space_it.next().?;
    var time_it = std.mem.split(time, ":]");
    const hour = try std.fmt.parseUnsigned(u8, time_it.next().?, 10);
    const minute = try std.fmt.parseUnsigned(u8, time_it.next().?, 10);
    const msg = space_it.rest();
    switch (msg[0]) {
        'G' => {
            const guard_id_text = std.mem.split(msg, "Guard #").next().?;
            const guard_id = try std.fmt.parseUnsigned(usize, guard_id_text, 10);
            return Msg{
                .event = Event{ .NewGuard = guard_id },
                .hour = hour,
                .minute = minute,
            };
        },
...
fn guard(allocator: *std.mem.Allocator, text: []const u8) !usize {
    var list = std.ArrayList([]const u8).init(allocator);
    var line_it = std.mem.split(text, "\n");
    while (line_it.next()) |line| {
        try list.append(line);
    }
    std.sort.sort([]const u8, list.toSlice(), lexicalLessThan);

    const SleepInfo = struct {
        best_minute: usize,
        per_minute_scores: [60]usize,
    };

    var sleep_table = std.AutoHashMap(usize, SleepInfo).init(allocator);
    ...
    var best_guard: usize = undefined;
    var global_best_minute: ?usize = null;
    var global_best_minute_amt: usize = undefined;

    var prev_info: ?Prev = null;
    for (list.toSlice()) |line| {
        const msg = try parseLine(line);

        if (prev_info) |prev| {
            var cur_guard = prev.guard;
            switch (prev.msg.event) {
                Event.Sleep => {
                    const get_or_put_result = try sleep_table.getOrPut(cur_guard);
                    if (!get_or_put_result.found_existing) {
                        get_or_put_result.kv.value = SleepInfo{
                            .best_minute = 0,
                            .per_minute_scores = [1]usize{0} ** 60,
                        };
                    }
                    var i: usize = prev.msg.minute;
                    while (i < msg.minute) : (i += 1) {
                        get_or_put_result.kv.value.per_minute_scores[i] += 1;
                        var best_minute = get_or_put_result.kv.value.best_minute;
                        if (get_or_put_result.kv.value.per_minute_scores[i] >
                            get_or_put_result.kv.value.per_minute_scores[best_minute])
                        {
                            best_minute = i;
                            get_or_put_result.kv.value.best_minute = i;
                        }
                        if (global_best_minute) |gbm| {
                            if (get_or_put_result.kv.value.per_minute_scores[best_minute] > global_best_minute_amt) {
                                global_best_minute = best_minute;
                                global_best_minute_amt = get_or_put_result.kv.value.per_minute_scores[best_minute];
                ...
                Event.NewGuard => |new_guard| cur_guard = new_guard,
                else => {},
            }
            prev_info = Prev{
                .guard = cur_guard,
            ...
            prev_info = Prev{
                .guard = msg.event.NewGuard,
                .msg = msg,
            };
        }
    }

    return global_best_minute.? * best_guard;
}
