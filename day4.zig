...
fn guard(allocator: *std.mem.Allocator, text: []const u8) !usize {
            ...
            switch (prev.msg.event) {
                Event.Sleep => {
                    ...
                    while (i < msg.minute) : (i += 1) {
                        get_or_put_result.kv.value.minutes_asleep += 1;
    ...
    var table_it = sleep_table.iterator();
    var most_minutes_asleep = table_it.next().?;
    while (table_it.next()) |next| {
        if (next.value.minutes_asleep > most_minutes_asleep.value.minutes_asleep)
            most_minutes_asleep = next;
    }
    var best_minute: usize = 0;
    const pms = &most_minutes_asleep.value.per_minute_scores;
    var i: usize = 1;
    while (i < 60) : (i += 1) {
        if (pms[i] > pms[best_minute])
            best_minute = i;
    }
    return most_minutes_asleep.key * best_minute;
}
